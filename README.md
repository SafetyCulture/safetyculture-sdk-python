...
* The Sync Users Tool to import a list of users from a spreadsheet into iAuditor and assign them to groups
...
This tool updates the organisation users in iAuditor from a CSV file. After running this tool, users in iAuditor and their corresponding groups will match the contents of the CSV file.
The groups specified in the CSV file must have been created in iAuditor before running the tool. If a group doesn't exist, the user will not be added to that group.
...
Adding an already invited user will log an error and have no effect.

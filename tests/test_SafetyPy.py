...
class SafetyPyTestCase(unittest.TestCase):
    ...
    def test_constructor_with_valid_api_token(self):
        ...
        try:
            sp.SafetyCulture(valid_token)
        except:
            self.fail("Encountered an unexpected exception with valid token.")

    def test_construct_correct_custom_headers(self):
        valid_token = '032d09de1ef9c43eb77f56da82ae23588d1564b9fa6f6f59e9a1849191ef1214'
        try:
            sc_client = sp.SafetyCulture(valid_token)

            expected_http_headers = {
                'User-Agent': 'safetyculture-python-sdk',
                'Authorization': 'Bearer ' + valid_token
            }

            self.assertEqual(sc_client.custom_http_headers, expected_http_headers)
        except:
            self.fail("Failed to construct the expected custom HTTP headers")

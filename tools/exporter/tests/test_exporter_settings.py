...
class ExporterTestCase(unittest.TestCase):
    ...
    def should_return_None_if_no_export_profile_mapping_was_given(self):
    ...
    def should_return_None_if_no_export_path_was_given(self):
        config_settings = [{}, None, '']
        for config_setting in config_settings:
            message = '{0} should cause None to be returned'.format(str(config_setting))
            self.assertIsNone(exp.get_export_path(logger, config_setting), msg=message)

    def should_use_user_supplied_unix_style_export_path_if_given(self):
    ...
    def should_use_user_supplied_windows_style_export_path_if_given(self):
        config_setting = {'export_options': {'export_path': r'C:\Users\Monty\Dropbox'}}
        self.assertEqual(exp.get_export_path(logger, config_setting), config_setting['export_options']['export_path'])

    def should_use_local_timezone_if_none_given(self):
        config_settings = [{}, None, '']
        for config_setting in config_settings:
    ...
    def should_use_valid_timezone_from_settings_if_given(self):
    ...
    def should_return_None_if_API_token_is_missing(self):
        config_settings = [{}, None, '']
        for config_setting in config_settings:
            message = '{0} should cause None to be returned'.format(str(config_setting))
            self.assertIsNone(exp.parse_api_token(logger, config_setting), msg=message)

    def should_return_None_if_API_token_has_invalid_format(self):
    ...
    def should_successfully_parse_a_valid_API_token(self):
    ...
    def should_use_default_if_sync_delay_setting_is_missing(self):
    ...
    def should_use_default_if_sync_delay_setting_is_invalid(self):
        config_settings = [{'sync_delay_in_seconds': 'abc'}, {'sync_delay_in_seconds': -1}]
        for config_setting in config_settings:
            self.assertEqual(exp.get_sync_delay(logger, config_setting), exp.DEFAULT_SYNC_DELAY_IN_SECONDS)

    def should_use_user_supplied_sync_delay_if_valid(self):
